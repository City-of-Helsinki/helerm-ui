...
export default class InfinityMenu extends Component {
  ...
  static defaultProps = {
    ...
    filter: (node, searchInput) =>
      node.name.toLowerCase().indexOf(searchInput.toLowerCase()) >= 0,
    headerContent: null,
    headerProps: {},
    maxLeaves: Infinity,
    onLeafMouseClick: () => {},
  ...
  onLoadMoreClick(tree, node, keyPath, event) {
    ...
    const parentPath = Object.assign([], keyPathArray).splice(
      0,
      keyPathArray.length - 2
    );
    const parentNode = _get(this.props.tree, parentPath);
    // set new max leaves - if none exist use component default property
    parentNode.maxLeaves = !parentNode.maxLeaves
      ? this.props.maxLeaves
      : parentNode.maxLeaves + this.props.maxLeaves;
    if (this.props.onNodeMouseClick) {
      const currLevel = Math.floor(keyPath.split('.').length / 2);
      this.props.onNodeMouseClick(event, tree, node, currLevel, keyPath);
    }
  ...
  onFilterConditionChange(option) {
    this.setState({ filterCondition: option.value }, () =>
      this.filterTree(
        ...
        this.props.tree,
        this.props.isDetailSearch
      )
    );
  }

  filterTree(searchInputs, tree, isDetailSearch) {
    const filters = isDetailSearch
      ? this.getDetailFilters(searchInputs)
      : searchInputs[0];
    ...
    if (!node.children) {
      ...
      if (filters.length) {
        if (this.props.isDetailSearch) {
          const snapshot = this.state.snapshots[node.id]
            ? this.state.snapshots[node.id]
            : null;
      ...
      if (shouldDisplay) {
        ...
        newNode.isSearchDisplay = true;
        newNode.maxLeaves = newNode.maxLeaves
          ? newNode.maxLeaves
          : this.props.maxLeaves;
        trees.push(newNode);
      }
  ...
  setDisplayTree(tree, prevs, curr, keyPath) {
    ...
    if (!curr.children) {
      ...
      const filteredChildren = parentNode.children.some(
        (child) => child.isSearchDisplay === true
      )
      ...
      if (shouldDisplay && parentMaxLeaves > relativeIndex) {
          ...
          prevs.push(
            <li
              ...
              className={
                ...
                (!curr.function ? ' new-leaf' : '')
              }
              onMouseDown={(e) =>
                this.props.onLeafMouseDown
                  ? this.props.onLeafMouseDown(e, curr)
                  : null
              }
              onMouseUp={(e) =>
                this.props.onLeafMouseUp
                  ? this.props.onLeafMouseUp(e, curr)
                  : null
              }
              onClick={(e) =>
                this.props.onLeafMouseClick
                  ? this.props.onLeafMouseClick(e, curr)
                  : null
              }
            >
        ...
        shouldDisplay &&
        relativeIndex === filteredChildren.length - 1
      ) {
        prevs.push(
          <li
            key={itemKey}
      ...
      if (
        (!curr.isOpen && !isSearching) ||
        (!curr.isSearchOpen && isSearching)
      ) {
        if (shouldDisplay) {
          if (curr.customComponent) {
            const nodeProps = {
        ...
        if (shouldDisplay) {
          if (curr.customComponent) {
            ...
            openedNode.push(
              React.createElement(currCustomComponent, nodeProps)
            );
          } else {
            openedNode.push(
              <div
      ...
      >
        <SearchInput
          ...
          setSearchInput={(event) =>
            this.props.setSearchInput(index, event.target.value)
          }
        />
        {isDetailSearch && (
          <div className='filters-detail-search-input-buttons'>
            {index + 1 < searchInputs.length && (
              <Select
                ...
                isDisabled={index > 0}
                placeholder='Ehto'
                value={FILTER_CONDITION_OPTIONS.find(
                  ({ value }) => value === this.state.filterCondition
                )}
                isClearable={false}
