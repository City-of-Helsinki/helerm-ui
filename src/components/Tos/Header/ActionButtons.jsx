...
import { EDIT, REVIEW, APPROVE, DRAFT, SENT_FOR_REVIEW, WAITING_FOR_APPROVAL, APPROVED } from '../../../constants';
...
const ActionButtons = ({
  ...
  const approvable = (
    <IsAllowed to={APPROVE}>
      <span>
        <ActionButton className='btn pull-right' type='primary' action={() => changeStatus(APPROVED)} label='Hyväksy' />
        <ActionButton
          className='btn pull-right'
          type='danger'
          action={() => changeStatus(DRAFT)}
