  ...
  const renderItemChanges = (previewItem) => {
    ...
    BULK_UPDATE_SEARCH_ADDITIONAL_FUNCTION_ATTRIBUTES.forEach((attribute) => {
      if (changed[attribute.value]) {
        ...
        changes.push(renderFunctionAttributeChange(item, attribute, changed[attribute.value], currentValue));
      }
    });
    ...
    if (!isEmpty(changed.attributes)) {
      keys(changed.attributes).forEach((attribute) => {
        ...
        changes.push(
          renderAttributeChange(item, attribute, changed.attributes[attribute], getAttributeName, currentValue),
        );
      });
    }

    if (!isEmpty(changed.phases)) {
      renderPhaseChanges(changed, item, changes, getAttributeName);
    }
  ...
  const renderItemErrors = (previewItem) => {
    ...
    if (errors.phases) {
      keys(errors.phases).forEach((phaseId) => {
        ...
        renderActionErrors(phaseError, phase, elem, getAttributeName);
      });
    }
