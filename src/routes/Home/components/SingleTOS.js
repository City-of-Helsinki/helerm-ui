  ...
  generatePhases(phases) {
    ...
    for(const key in phases) {
      if (phases.hasOwnProperty(key)) {
        const modifiedDateTime = this.formatDateTime(phases[key].modified_at);
        const formattedDate = formatDate(modifiedDateTime.date, 'DD.MM.YYYY');
        const idName = "#"+phases[key].id;
        ...
        phaseElements.push(
          <div key={key} className="col-xs-12 box">
            <div className="col-xs-12 space-between">
              <span className="phase-title"><i className="fa fa-info-circle" aria-hidden="true"></i> {phases[key].name}</span>
              {
                phases[key].actions.length !== 0
                &&
                <button type="button" onClick={() => this.props.togglePhaseVisibility(key, phases[key].is_open)}>
                  <span className={"fa black-icon " + (phases[key].is_open ? 'fa-minus' : 'fa-plus')}  aria-hidden="true" />
                </button>
              }
            </div>
            {
      ...
      return (
        <div className="col-xs-12 single-tos-container">
          ...
          <div className="button-row col-xs-12">
            <button className="btn btn-default btn-sm pull-right" onClick={() => this.props.setPhasesVisibility(selectedTOSData.phases, true)}>Avaa kaikki</button>
            <button className="btn btn-default btn-sm pull-right" onClick={() => this.props.setPhasesVisibility(selectedTOSData.phases, false)}>Pienennä kaikki</button>
          </div>
          { phases }
        </div>
      );
